"""Safe staging install for Shader Health Inspector updates."""
...
def resolve_default_install_root(start: Path | None = None) -> Path:
    ...
    if start is None:
        ...
        start = Path(shader_health.__file__).resolve().parent
...
def _preserved_config_paths() -> tuple[Path, ...]:
    ...
    shader_health_dir = Path.home() / USER_CONFIG_DIRNAME
    defaults = (
        shader_health_dir / STUDIO_CONFIG_FILENAME,
        shader_health_dir / USER_CONFIG_FILENAME,
    )
