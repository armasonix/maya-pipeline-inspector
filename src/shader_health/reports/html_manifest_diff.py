...
from shader_health.reports.html_report import _attr, _format_value, _stylesheet, _text
...
def build_html_manifest_diff(diff_payload: Mapping[str, Any]) -> str:
    ...
    html = [
        ...
        f"<title>{_text('Maya Shader Health Manifest Diff')}</title>",
...
def _render_header(payload: JsonDict) -> str:
    ...
    return "\n".join(
        [
            ...
            '<div class="hero-top">',
            '<div class="hero-copy">',
            f"<h1>{_text('Maya Shader Health Manifest Diff')}</h1>",
            (
...
def _render_footer() -> str:
    return "\n".join(
        [
            ...
            "<p>Generated by Maya Shader Health Inspector · manifest diff report</p>",
            "</footer>",
        ]
    )
