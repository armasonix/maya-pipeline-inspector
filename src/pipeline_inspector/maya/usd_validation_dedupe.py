...
_MATERIAL_FROM_FLATTENED = re.compile(
    "_mtl_([^_/]+(?:_[^_/]+)?)(?:_[Vv][Rr]ay|_[Uu]sd|_|$)", re.IGNORECASE
)
_TEXTURE_SUFFIX_RE = re.compile("(?:_place2dtexture\\d*|_bitmap|_tex)$", re.IGNORECASE)
_MTL_SEGMENT_RE = re.compile("_[Mm][Tt][Ll]_")


def dedupe_validation_results(
    snapshot: GraphSnapshot, results: list[RuleResult]
) -> list[RuleResult]:
...
def _colorspace_group_key(
    result: RuleResult, nodes_by_id: dict[str, NodeSnapshot]
) -> tuple[str, str, str]:
...
def _resolve_texture_identity(result: RuleResult, node: NodeSnapshot | None) -> str:
    candidates: list[str] = []
    if node is not None:
        candidates.extend([str(node.full_name or ""), str(node.name or "")])
    candidates.extend([str(result.node or ""), str(result.target_id or "")])
...
def _normalize_texture_identity(text: str) -> str:
    ...
    while True:
        ...
        if stripped == segment:
            break
        segment = stripped
    segment = re.sub("_place2dtexture\\d*$", "", segment, flags=re.IGNORECASE)
    segment = re.sub("_bitmap$", "", segment, flags=re.IGNORECASE)
    segment = re.sub("_tex$", "", segment, flags=re.IGNORECASE)
    return segment.casefold()


def _resolve_material_for_result(result: RuleResult, node: NodeSnapshot | None) -> str:
...
def _colorspace_node_score(result: RuleResult, nodes_by_id: dict[str, NodeSnapshot]) -> int:
    ...
    if (
        "/usdpreviewsurface/" in lowered
        and "bitmap" not in lowered
        and ("place2dtexture" not in lowered)
    ):
        return 100
