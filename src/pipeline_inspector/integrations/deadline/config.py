"""Deadline 10 on-prem configuration for Pipeline Inspector farm integration."""
...
from pipeline_inspector.maya.validation_pipeline import packaged_profile_path

DEFAULT_PROFILE_ID = "deadline_critical"
DEFAULT_API_URL = "http://localhost:8081"
DEFAULT_TIMEOUT_SECONDS = 30.0

_ENV_API_URL = "PIPELINE_INSPECTOR_DEADLINE_API_URL"
_ENV_TIMEOUT = "PIPELINE_INSPECTOR_DEADLINE_TIMEOUT"
_ENV_PROFILE_ID = "PIPELINE_INSPECTOR_DEADLINE_PROFILE_ID"
_ENV_PROFILE_PATH = "PIPELINE_INSPECTOR_DEADLINE_PROFILE_PATH"
_ENV_MAYAPY = "PIPELINE_INSPECTOR_DEADLINE_MAYAPY"
_ENV_REPO_ROOT = "PIPELINE_INSPECTOR_DEADLINE_REPO_ROOT"
_ENV_QUEUE = "PIPELINE_INSPECTOR_DEADLINE_QUEUE"
...
def _env_get(values: Mapping[str, str], key: str, default: str | None = None) -> str | None:
    value = values.get(key)
    if value:
        return value
    legacy_key = _LEGACY_ENV.get(key)
    if legacy_key:
...
class DeadlineConfig:
    ...
    @classmethod
    def from_env(cls, environ: Mapping[str, str] | None = None) -> DeadlineConfig:
        """Load config from ``PIPELINE_INSPECTOR_DEADLINE_*`` environment variables."""
        ...
        return cls(
            ...
            timeout_seconds=_optional_float(
                _env_get(values, _ENV_TIMEOUT),
                DEFAULT_TIMEOUT_SECONDS,
            ),
            profile_id=_env_get(values, _ENV_PROFILE_ID, DEFAULT_PROFILE_ID) or DEFAULT_PROFILE_ID,
            profile_path=profile_path,
            mayapy=_env_get(values, _ENV_MAYAPY, "mayapy") or "mayapy",
            repo_root=repo_root,
            queue=_optional_str(_env_get(values, _ENV_QUEUE)),
