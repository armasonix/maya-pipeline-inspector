"""Deadline job templates and farm submit API for Pipeline Inspector validation."""
...
class PipelineInspectorValidationJobResult:
...
def submit_pipeline_inspector_validation_job(
    *,
    ...
    """Submit a Deadline utility job that runs Pipeline Inspector validation on a worker.
    ...
    return PipelineInspectorValidationJobResult(
        job_id=job_id,
        report_path=report_path,
        plugin=plugin,
...
def _base_job_info(
    *,
    ...
    job_info: dict[str, Any] = {
        "Name": job_name or f"Pipeline Inspector | {scene_path.name}",
        "Plugin": plugin,
