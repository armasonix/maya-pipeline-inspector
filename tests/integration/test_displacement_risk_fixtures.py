"""Fixture-driven integration tests for displacement risk enrichment and rules."""
...
DISPLACEMENT_FIXTURE_CASES = (
    "displacement_risk_safe",
    "displacement_risk_high_vray",
)
...
@pytest.mark.parametrize("fixture_stem", DISPLACEMENT_FIXTURE_CASES)
def test_displacement_fixture_matches_expectations_for_all_profiles(fixture_stem: str):
...
@pytest.mark.parametrize("fixture_stem", DISPLACEMENT_FIXTURE_CASES)
def test_displacement_fixture_snapshot_round_trips_after_enrichment(fixture_stem: str):
    snapshot = load_snapshot_fixture(fixture_stem)
    run = run_validation(snapshot, profile_id="supervisor_full", scan_scope="scene")
    assert run.snapshot.materials
    restored = GraphSnapshot.from_dict(run.snapshot.to_dict())
    assert restored.materials[0].displacement_metadata is not None
